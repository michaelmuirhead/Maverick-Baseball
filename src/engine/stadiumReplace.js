...
export function newParkCost(spec) {
...
export function buildNewStadium(state, spec) {
    ...
    if (liquid < cost * 0.4)
        return { ok: false, reason: `Need at least $${Math.round(cost * 0.4 / 1e6)}M between team and owner cash for a 40% down payment` };
