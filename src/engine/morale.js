...
export function initMorale(state) {
...
export function updateWeeklyMorale(state, _rng) {
    for (const pid of Object.keys(state.players)) {
        const p = state.players[pid];
        if (p.retired)
            continue;
...
export function rollDemands(state, rng) {
    for (const pid of Object.keys(state.players)) {
        const p = state.players[pid];
        if (!p.contract || p.retired)
            continue;
        p.demands = p.demands || {};
