...
export const GM_PHILOSOPHIES = ['rebuilding', 'retooling', 'contending', 'win_now', 'balanced'];
export function assignGMPhilosophy(rng, franchise) {
    const market = MARKETS[franchise.market];
    if (market.tier === 'mega')
        return rng.pick(['contending', 'win_now', 'balanced']);
    if (market.tier === 'large')
        return rng.pick(['contending', 'retooling', 'balanced', 'win_now']);
    if (market.tier === 'mid')
        return rng.pick(['retooling', 'balanced', 'contending', 'rebuilding']);
    return rng.pick(['rebuilding', 'retooling', 'balanced']);
}
export function effectivePhilosophy(state, fid) {
    ...
    const divRivals = Object.keys(FRANCHISES).filter((id) => FRANCHISES[id].lg === FRANCHISES[fid].lg && FRANCHISES[id].div === FRANCHISES[fid].div && id !== fid);
    ...
    if (winPct > 0.58 || (winPct > 0.52 && gamesBack <= 2)) {
        if (base === 'rebuilding')
            return 'retooling';
        if (base === 'retooling' || base === 'balanced')
            return 'contending';
        return base;
    }
    if (winPct < 0.42 && gamesBack > 8) {
        if (base === 'win_now' || base === 'contending')
            return 'retooling';
        if (base === 'balanced')
            return 'rebuilding';
        return base;
...
export function tradeValue(player) {
    ...
    if (player.contract.status === 'pre-arb')
        baseVal *= 1.4;
    else if (player.contract.status === 'arb')
        baseVal *= 1.15;
    if (player.contract.status === 'extension') {
        if (r >= 55 && player.contract.years >= 3)
            baseVal *= 1.1;
        if (r < 50 && player.contract.years >= 3)
            baseVal *= 0.75;
    }
    if (player.contract.ntc)
        baseVal *= 0.85;
    return Math.max(0, Math.round(baseVal));
}
export function estimateFairAAV(rating, age) {
    let base;
    if (rating >= 70)
        base = 40_000_000;
    else if (rating >= 60)
...
export function cashToTradeValue(dollars) {
    return Math.round(dollars / 2_000_000);
}
export function evaluateTradeForAI(state, aiFid, givingIds, receivingIds, aiGivingCash = 0, aiReceivingCash = 0) {
    ...
    if (giving.length === 0 && aiGivingCash === 0)
        return { accepted: false, reason: 'Empty package' };
    if (receiving.length === 0 && aiReceivingCash === 0)
        return { accepted: false, reason: 'Empty package' };
    ...
    for (const p of receiving) {
        ...
        if (philosophy === 'rebuilding' || philosophy === 'retooling') {
            if (p.age <= 25)
                recvMult += 0.08 * (tv / Math.max(1, receivingValue));
            if (p.age >= 32)
                recvMult -= 0.12 * (tv / Math.max(1, receivingValue));
            if (p.contract?.status === 'pre-arb')
                recvMult += 0.06 * (tv / Math.max(1, receivingValue));
        }
        if (philosophy === 'contending' || philosophy === 'win_now') {
            if (p.ratings.overall >= 55 && p.age >= 27 && p.age <= 33)
                recvMult += 0.08 * (tv / Math.max(1, receivingValue));
            if (p.potential - p.ratings.overall > 12)
                recvMult -= 0.06 * (tv / Math.max(1, receivingValue));
        }
    }
    for (const p of giving) {
        ...
        if (tv <= 0)
            continue;
        if (philosophy === 'rebuilding' && p.age <= 26 && p.ratings.overall >= 55)
            giveMult += 0.15 * (tv / Math.max(1, givingValue));
        if (philosophy === 'win_now' && p.age >= 30 && p.ratings.overall >= 55)
            giveMult -= 0.08 * (tv / Math.max(1, givingValue));
    }
    ...
    const margin = philosophy === 'win_now' ? -3 :
...
export function describeTradeRejection(receiving, giving, philosophy) {
    const gap = giving - receiving;
    if (gap > 80)
        return 'Not in the same zip code. Try again.';
    if (gap > 40)
        return "That's a non-starter for us.";
    if (philosophy === 'rebuilding')
        return "We're looking for younger pieces with more control.";
    if (philosophy === 'win_now')
        return "We need major-league help right now, not projects.";
    if (philosophy === 'contending')
        return "We'd need more proven talent to move that player.";
    return "Values don't line up for us.";
}
export function suggestCounter(state, _aiFid, userFid, _givingIds, receivingIds, evalResult) {
...
export function executeTradeWithCash(state, fromFid, toFid, fromPlayerIds, toPlayerIds, fromCash, toCash) {
...
export function executeTrade(state, fromFid, toFid, fromPlayerIds, toPlayerIds) {
    const s = {
        ...state,
    ...
    function moveOne(s, pid, srcFid, dstFid) {
        const wasActive = s.rosters[srcFid]?.includes(pid);
        let level = null;
        if (s.minorRosters?.[srcFid]) {
            if (s.minorRosters[srcFid].aaa.includes(pid))
                level = 'aaa';
            else if (s.minorRosters[srcFid].aa.includes(pid))
                level = 'aa';
            else if (s.minorRosters[srcFid].a.includes(pid))
                level = 'a';
        }
        ...
        if (wasActive) {
            ...
            if (!s.rosters[dstFid].includes(pid))
                s.rosters[dstFid].push(pid);
        }
        else if (level) {
            ...
            if (!dstMin[level].includes(pid))
                dstMin[level].push(pid);
        }
    }
    for (const pid of fromPlayerIds)
        moveOne(s, pid, fromFid, toFid);
    for (const pid of toPlayerIds)
        moveOne(s, pid, toFid, fromFid);
...
export function simAITradeActivity(state, rng) {
    ...
    for (let i = 0; i < attempts; i++) {
        ...
        const complementary = ((philA === 'contending' || philA === 'win_now') && (philB === 'rebuilding' || philB === 'retooling')) ||
...
export function tradeWindowOpen(state) {
    if (state.phase === 'offseason' && state.day >= -100 && state.day <= -30)
        return true;
    if (state.phase === 'regular_season' && state.day <= 120)
        return true;
    return false;
}
export function tradeWindowLabel(state) {
    if (!tradeWindowOpen(state)) {
        if (state.day > 120 && state.day <= 183)
            return 'Deadline has passed';
        if (state.phase === 'postseason')
            return 'Postseason - window closed';
