...
function baseRate(p) {
    ...
    if (p.isPitcher)
        r *= 1.25;
    if (p.isPitcher && p.pos === 'SP')
        r *= 1.1;
    if (['C', 'CF', 'SS'].includes(p.pos))
        r *= 1.15;
    return r;
}
function rollInjuryType(rng, p) {
    const pool = p.isPitcher ? INJURY_TYPES_PITCHER : INJURY_TYPES_HITTER;
    const weights = pool.map((i) => (i.severity === 'day_to_day' ? 40 :
        i.severity === 'il_10' ? 30 :
            i.severity === 'il_15' ? 22 :
                6));
...
export function rollDailyInjuries(state, rng) {
    if (state.phase !== 'regular_season' && state.phase !== 'postseason')
        return state;
...
export function applyInjury(state, rng, p) {
...
export function recoverPlayer(_state, p) {
    if (!p.injury)
        return;
...
export function injuredPlayers(state, fid) {
    return state.rosters[fid]
        .map((pid) => state.players[pid])
        .filter((p) => p && p.health !== 'healthy');
}
export function healthyPlayers(state, fid) {
    return state.rosters[fid]
        .map((pid) => state.players[pid])
        .filter((p) => p && p.health === 'healthy');
}
