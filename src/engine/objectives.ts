...
const OBJECTIVE_PENALTIES = { wins: 8, playoffs: 8, profit: 6, attendance: 4 } as const;
 ...
 */
export function setSeasonObjectives(state: GameState): OwnerObjectives {
  const fid = state.userFranchiseId;
  const f = FRANCHISES[fid];
  const market = MARKETS[f.market];

  // Win target by market tier
  const winTarget = ({ mega: 92, large: 87, mid: 82, small: 78 } as const)[market.tier];
 ...
 */
export function evaluateObjectives(state: GameState) {
