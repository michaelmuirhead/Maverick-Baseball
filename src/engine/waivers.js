...
export function processWaivers(state, rng) {
    if (!state.waiverWire)
        return;
    const remaining = [];
    for (const w of state.waiverWire) {
        ...
        if (state.day < w.expiresOnDay) {
            ...
            const interested = candidates.filter((fid) => {
                ...
                if (phil === 'rebuilding' && p.ratings.overall < 50)
                    return rng.chance(0.4);
                if (p.ratings.overall >= 60)
                    return rng.chance(0.5);
                if (p.ratings.overall >= 50)
        ...
        if (state.day >= w.expiresOnDay) {
            ...
            if (optionsUsed < 3) {
                ...
                if (!minors.aaa.includes(w.playerId))
                    minors.aaa.push(w.playerId);
                state.rosters[w.fromFid] = (state.rosters[w.fromFid] || []).filter((id) => id !== w.playerId);
            }
            else {
                // Out of options → FA
...
export function isOutOfOptions(p) {
