...
function isEligible(p) {
    ...
    if (p.age > 25)
        return false;
    if (p.contract)
        return false; // already on 40-man (roughly)
    return true;
}
function exposedFor(state, fid) {
...
export function startRule5Draft(state, _rng) {
    ...
    for (const fid of fids) {
        for (const id of exposedFor(state, fid))
            eligibleSet.add(id);
    }
    return {
        year: state.season,
...
function performPick(state, fid, playerId) {
    ...
    if (playerId) {
        ...
        if (p) {
            ...
            if (state.finances[fid])
                state.finances[fid].teamCash -= R5_FEE;
...
export function userRule5Pick(state, playerId) {
    const draft = state.rule5;
    ...
    if (fid !== state.userFranchiseId)
        return false;
    if (!draft.eligible.includes(playerId))
        return false;
...
export function autoRule5Pick(state, rng) {
    const draft = state.rule5;
    if (!draft || draft.complete)
        return false;
    const fid = currentTeam(draft);
    ...
    if (fid === state.userFranchiseId)
        return false; // don't pick for user
    ...
    if (phil === 'rebuilding')
        willPick = rng.chance(0.75);
    if (phil === 'win_now')
        willPick = rng.chance(0.3);
...
export function autoCompleteRule5(state, rng) {
    const draft = state.rule5;
    if (!draft)
        return;
    let safety = 50;
...
export function simAIRule5Until(state, rng) {
    const draft = state.rule5;
    if (!draft || draft.complete)
        return;
    let safety = 40;
...
export function rule5CurrentTeam(state) {
