 ...
 */
export function generateDraftClass(rng, year, teamCount) {
    const pool = [];
    ...
    for (let i = 0; i < total; i++) {
        ...
        const pos = rng.chance(0.4)
            ? rng.pick(['SP', 'SP', 'SP', 'RP'])
            : rng.pick(['SS', '2B', 'CF', 'C', '3B', '1B', 'LF', 'RF']);
        ...
        if (p.ratings.contact !== undefined)
            p.ratings.contact = Math.max(20, p.ratings.contact - rng.int(5, 12));
        if (p.ratings.power !== undefined)
            p.ratings.power = Math.max(20, p.ratings.power - rng.int(5, 10));
        if (p.ratings.fastball !== undefined)
            p.ratings.fastball = Math.max(20, p.ratings.fastball - rng.int(5, 10));
 ...
 */
export function buildDraftOrder(state) {
...
function slotValue(overallPick) {
    if (overallPick <= 30)
        return SLOT_POOL[overallPick - 1];
    if (overallPick <= 60)
        return Math.round(1_300_000 - (overallPick - 30) * 25_000);
    if (overallPick <= 100)
        return Math.round(700_000 - (overallPick - 60) * 8_000);
    return Math.max(150_000, Math.round(380_000 - (overallPick - 100) * 2_500));
}
export function startDraft(state, rng, year) {
    const fids = Object.keys(FRANCHISES);
    ...
    const teamBonusPools = {};
    for (const fid of fids) {
        let pool = 0;
        for (const pick of picks)
            if (pick.franchiseId === fid)
                pool += slotValue(pick.overall);
...
function prospectValue(p, philosophy) {
...
export function aiPick(state, draft, _rng) {
...
export function makePick(state, draft, playerId) {
    ...
    state.prospects = state.prospects || [];
    if (!state.prospects.includes(playerId))
        state.prospects.push(playerId);
...
export function autoPickUntilUser(state, draft, userFid, rng) {
    while (!draft.complete) {
        const pick = draft.picks[draft.currentPickIndex];
        if (!pick)
            break;
        if (pick.franchiseId === userFid)
            break;
...
export function autoCompleteDraft(state, draft, rng) {
    while (!draft.complete) {
        const playerId = aiPick(state, draft, rng);
        if (!playerId)
            break;
