...
export function emptyDepthChart() {
    const chart = {};
...
export function autoFillDepthChart(state, fid) {
...
export function assignBullpenRoles(state, fid) {
...
export function setUserDepthAt(state, fid, pos, orderedIds) {
    state.depthCharts = state.depthCharts || {};
    state.depthCharts[fid] = state.depthCharts[fid] || emptyDepthChart();
    state.depthCharts[fid][pos] = orderedIds;
}
export function setBullpenRoleAction(state, playerId, role) {
    const p = state.players[playerId];
    if (!p)
        return { ok: false, reason: 'no player' };
    if (!['RP', 'CL'].includes(p.pos))
        return { ok: false, reason: 'not a reliever' };
    p.bullpenRole = role;
    return { ok: true };
}
/** Initialize all teams' depth charts at world init. */
export function initAllDepthCharts(state) {
    state.depthCharts = state.depthCharts || {};
