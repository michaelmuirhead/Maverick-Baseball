...
// ===== Extensions =====
export function offerExtension(state, playerId, years, aav) {
    const p = state.players[playerId];
    if (!p || !p.contract)
        return { ok: false, accepted: false, reason: 'No contract to extend' };
    if (p.franchiseId !== state.userFranchiseId)
        return { ok: false, accepted: false, reason: 'Not your player' };
    ...
    if (accepted) {
        ...
        if (p.demands)
            p.demands.wantsExtension = false;
...
export function fileArbitrations(state, rng) {
    const cases = [];
...
export function resolveArbitrations(state, cases, rng) {
...
export function tenderQO(state, playerId) {
    const p = state.players[playerId];
    if (!p)
        return { ok: false, reason: 'No player' };
    if (p.franchiseId !== state.userFranchiseId)
        return { ok: false, reason: 'Not your player' };
    if (p.ratings.overall < 60)
        return { ok: false, reason: 'Not eligible (need OVR 60+)' };
...
export function awardCompPick(state, playerId, signedTeamFid) {
    const qo = state.qualifyingOffers?.find((q) => q.playerId === playerId);
    if (!qo || qo.signedElsewhere)
        return;
