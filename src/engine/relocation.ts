...
export function relocateFranchise(state: GameState, spec: RelocationSpec): { ok: boolean; reason?: string } {
  ...
  if (spec.name.length < 2 || spec.abbr.length !== 3) return { ok: false, reason: 'Need 2+ char name and 3-char abbr' };
  ...
  f.city = spec.city;
  if (spec.state) f.state = spec.state;
  f.name = spec.name;
  f.abbr = spec.abbr;
  f.color = spec.color;
  if (spec.secondaryColor) f.secondaryColor = spec.secondaryColor;
  f.park = `${spec.city} Park`;
  // Reset park factors to neutral
  f.pf = 100;
  f.pfHR = 100; f.pfDouble = 100; f.pfTriple = 100; f.pfBABIP = 100;
