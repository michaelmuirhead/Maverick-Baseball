...
function ensureSeasonEntry(p, season, fid, isPitcher) {
...
function recomputeRates(s, isPitcher) {
    if (isPitcher) {
        const ps = s;
 ...
 */
function todaysLineup(state, fid) {
...
function gameHitterLine(rng, p) {
    ...
    if (AB === 0)
        return { AB: 0, H: 0, D: 0, T: 0, HR: 0, BB: 0, SO: 0, R: 0, RBI: 0, SB: 0 };
    ...
    for (let i = 0; i < H; i++) {
        const r = rng.next();
        if (r < hrChance * 4.0)
            HR++; // boost since only "hits" are eligible
        else if (r < hrChance * 4.0 + dblChance * 2.5)
            D++;
        else if (r < hrChance * 4.0 + dblChance * 2.5 + 0.018)
            T++;
    }
...
function gamePitcherLine(rng, p, runsAllowed, isWin) {
    const ovr = p.ratings.overall;
...
export function recordGameStats(state, game, rng) {
    if (!game.played || !game.result)
        return;
    if (state.phase === 'postseason')
        return recordPostseasonStats(state, game, rng);
    ...
    for (const fid of [homeFid, awayFid]) {
        ...
        for (const h of lineup.hitters) {
            ...
            if (!hSplits[vsKey])
                hSplits[vsKey] = emptySplit();
            if (!hSplits[homeKey])
                hSplits[homeKey] = emptySplit();
            hSplits.byMonth = hSplits.byMonth || {};
            if (!hSplits.byMonth[monthKey])
                hSplits.byMonth[monthKey] = emptySplit();
        ...
        if (lineup.sp) {
            ...
            if (!pSp[pHomeKey])
                pSp[pHomeKey] = emptyPSplit();
            pSp.byMonth = pSp.byMonth || {};
            if (!pSp.byMonth[pMonthKey])
                pSp.byMonth[pMonthKey] = emptyPSplit();
...
function recordPostseasonStats(state, game, rng) {
    if (!game.played || !game.result)
        return;
    const homeFid = game.home;
    const awayFid = game.away;
