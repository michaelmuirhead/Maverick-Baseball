...
export function startSpringTraining(state) {
 ...
 */
export function executeCuts(state, _rng) {
    for (const fid of Object.keys(FRANCHISES)) {
        const roster = state.rosters[fid] || [];
        if (roster.length <= ACTIVE_ROSTER_TARGET)
            continue;
    ...
    if (state.springTraining)
        state.springTraining.active = false;
    state.news.unshift({
