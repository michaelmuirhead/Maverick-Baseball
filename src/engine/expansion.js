...
export function generateExpansionTeam(rng, opts = {}) {
    ...
    const cityChoice = opts.city
        ? EXPANSION_CITIES.find((c) => c.city === opts.city) ?? rng.pick(cityPool)
        : rng.pick(cityPool);
    ...
    const marketTier = opts.marketTier || cityChoice.market || 'mid';
    return {
...
export function chooseAutoTeamSlot(userLg, userDiv) {
    const oppLg = userLg === 'AL' ? 'NL' : 'AL';
    return { lg: oppLg, div: userDiv };
}
/** Register an expansion franchise in the global FRANCHISES and MARKETS maps. */
export function registerExpansionFranchise(id, config) {
