...
function startingPitcher(state, fid) {
...
function reliever(state, fid, situation = 'middle') {
...
function rollAtbat(rng, batter, pitcher) {
    ...
    const pitches = [];
    function addPitch(kind) { pitches.push(kind); }
    ...
    if (r < kProb)
        return { outcome: 'K', pitches: pitchSeq };
    if (r < kProb + bbProb)
        return { outcome: 'BB', pitches: pitchSeq };
    if (r < kProb + bbProb + hbpProb)
        return { outcome: 'HBP', pitches: pitchSeq };
    ...
    if (inPlay < babip) {
        ...
        if (hitRoll < hrChance)
            return { outcome: 'HR', pitches: pitchSeq };
        if (hitRoll < hrChance + dblChance)
            return { outcome: '2B', pitches: pitchSeq };
        if (hitRoll < hrChance + dblChance + tplChance)
            return { outcome: '3B', pitches: pitchSeq };
        return { outcome: '1B', pitches: pitchSeq };
    }
    return { outcome: rng.chance(0.55) ? 'GO' : 'FO', pitches: pitchSeq };
}
function applyAtbat(outcome, bases) {
    const b = { ...bases };
    let runs = 0;
    let outs = 0;
    switch (outcome) {
...
export function startLiveGame(state, game) {
    return {
        gameId: game.id,
        homeFid: game.home,
        awayFid: game.away,
...
export function simNextHalfInning(state, live, rng) {
    if (live.finished)
        return live;
    ...
    let pitcher;
    if (live.inning <= 6)
        pitcher = startingPitcher(state, fieldingFid);
    ...
    let outs = 0;
    let batterIdx = ((live.log.filter((a) => (live.half === 'top' && a.half === 'top') || (live.half === 'bot' && a.half === 'bot')).length) % lineupOrder.length);
    let runsThisHalf = 0;
    let safety = 30;
    while (outs < 3 && safety-- > 0) {
        ...
        runsThisHalf += r.runs;
        if (live.half === 'top')
            live.scoreAway += r.runs;
        else
            live.scoreHome += r.runs;
    ...
    if (live.inning > 15)
        live.finished = true; // safety bail
    return live;
}
export function simFullLiveGame(state, game, rng) {
...
export function describeAtbat(a) {
...
export function homeTeamAbbr(state, live) {
    return FRANCHISES[live.homeFid]?.abbr || live.homeFid;
}
export function awayTeamAbbr(state, live) {
