 ...
 */
export function updateWeeklyStreaks(state: GameState, rng: RNG) {
...
export function resetStreaks(state: GameState) {
  for (const pid of Object.keys(state.players)) {
    if (state.players[pid].streakMod) state.players[pid].streakMod = 0;
  }
}
...
export function streakLabel(streakMod: number | undefined): string {
  if (!streakMod) return 'Even';
  if (streakMod >= 5) return '🔥 Hot';
  if (streakMod >= 2) return 'Warm';
  if (streakMod <= -5) return '❄️ Cold';
  if (streakMod <= -2) return 'Cool';
  return 'Even';
}
