import type { Player, GameState, DefensiveMetrics, Position } from './types';

export function computeDefensiveMetrics(p: Player, inningsByPos: Partial<Record<Position, number>>): DefensiveMetrics {
...
export function rollSeasonDefensiveMetrics(state: GameState): GameState {
  const players: Record<string, Player> = { ...state.players };
  for (const id of Object.keys(players)) {
    const p = players[id];
    if (p.retired || !p.franchiseId) continue;
    if (p.isPitcher && p.pos !== 'C') continue;
    const last = (p.statsHistory || [])[(p.statsHistory || []).length - 1];
    if (!last?.hitter) continue;
    const games = last.hitter.G || 0;
    if (games < 30) continue;
    const ip = games * 8.5;
    const inningsByPos: Partial<Record<Position, number>> = { [p.pos]: ip };
...
export function catcherEffectFor(state: GameState, franchiseId: string): { framing: number; blocking: number; throwing: number } {
  const catchers = Object.values(state.players).filter(p => p.franchiseId === franchiseId && p.pos === 'C' && !p.retired && !p.injury);
  if (catchers.length === 0) return { framing: 0, blocking: 0, throwing: 22 };
  catchers.sort((a, b) => (b.ratings.overall || 0) - (a.ratings.overall || 0));
  const top = catchers[0];
  const r: any = top.ratings;
  const fld = r.fielding ?? 50;
...
export function outfieldArmFor(state: GameState, franchiseId: string): number {
  const ofs = Object.values(state.players).filter(p => p.franchiseId === franchiseId && (p.pos === 'LF' || p.pos === 'CF' || p.pos === 'RF') && !p.retired && !p.injury);
  if (ofs.length === 0) return 50;
  const avgArm = ofs.reduce((s, p) => s + ((p.ratings as any).armStr || 50), 0) / ofs.length;
  return avgArm;
}
