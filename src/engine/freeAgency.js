...
export function processOffseasonEligibility(state) {
    ...
    for (const pid of state.freeAgents) {
        ...
        if (!p)
            continue;
        if (state.freeAgency.listings.find((l) => l.playerId === pid))
            continue;
...
export function topFreeAgents(state, n) {
...
export function bidScore(bid, listing, p) {
    ...
    const fitBonus = p.age >= 30 && bid.philosophy === 'win_now' ? 1.05 :
        p.age <= 27 && bid.philosophy === 'rebuilding' ? 0.85 :
            1.0;
    return totalValue * askMatch * yearsMatch * fitBonus;
}
export function simAIFreeAgentBids(state, rng) {
    ...
    for (const listing of fa.listings) {
        ...
        for (const fid of fids) {
            if (!rng.chance(0.06 * interestMult))
                continue;
            const philosophy = effectivePhilosophy(state, fid);
            if (philosophy === 'rebuilding' && player.age >= 31)
                continue;
            if (philosophy === 'win_now' && player.age <= 24 && player.ratings.overall < 55)
                continue;
            const fin = state.finances[fid];
            if (!fin)
                continue;
            const currentPayroll = state.rosters[fid].reduce((s, pid) => s + (state.players[pid]?.contract?.salary || 0), 0);
            const revenueProxy = fin.tvValue + fin.sponsors + fin.nameRightsValue + 75_000_000;
            if (currentPayroll > revenueProxy * 0.85)
                continue;
            if (fin.teamCash < listing.asking * 0.5)
                continue;
            const fairAAV = estimateFairAAV(player.ratings.overall, player.age);
            let aggressiveness = philosophy === 'win_now' ? rng.float(0.95, 1.15) :
            ...
            if (fin.teamCash > 300_000_000)
                aggressiveness *= 1.10;
            if (fin.teamCash > 500_000_000)
                aggressiveness *= 1.05;
...
export function userPlaceBid(state, fid, playerId, aav, years) {
    const fa = state.freeAgency;
    if (!fa)
        return { ok: false, reason: 'Free agency has not opened yet' };
    const fin = state.finances[fid];
    if (!fin || fin.teamCash < aav)
        return { ok: false, reason: 'Not enough cash on hand' };
    const listing = fa.listings.find((l) => l.playerId === playerId);
    if (!listing)
        return { ok: false, reason: 'Player not available' };
    if (listing.signedBy)
        return { ok: false, reason: 'Player already signed' };
    ...
    const bid = {
        ...
        years,
        total: aav * years,
        philosophy: (state.gmPhilosophies?.[fid] || 'balanced'),
        dayPlaced: state.day,
    };
    listing.bids = listing.bids.filter((b) => b.franchiseId !== fid);
    listing.bids.push(bid);
...
export function signFreeAgent(state, listing, bid) {
...
export function closeFreeAgencyWindow(state) {
    if (!state.freeAgency)
        return;
 ...
 */
export function simUserGMFreeAgentBids(state, rng) {
    const fa = state.freeAgency;
    if (!fa || !fa.open || !state.delegateFA)
        return state;
    ...
    const positionsToFill = ['C', '1B', '2B', '3B', 'SS', 'LF', 'CF', 'RF', 'DH', 'SP', 'RP'];
    ...
    if (currentPayroll > revenueProxy * 0.8)
        return state;
    ...
    for (const pos of weakest) {
        ...
        // Don't double-bid
        if (listing.bids.find((b) => b.franchiseId === fid))
            continue;
        // Compute fair AAV
        const fairAAV = estimateFairAAV(player.ratings.overall, player.age);
