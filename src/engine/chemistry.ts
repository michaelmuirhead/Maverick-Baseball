...
import type { GameState, TeamChemistry } from './types';
import type { RNG } from './rng';
import { FRANCHISES } from './franchises';

export function initChemistry(state: GameState) {
...
export function recomputeLeaders(state: GameState, fid: string) {
  if (!state.chemistry) state.chemistry = {};
...
export function updateWeeklyChemistry(state: GameState, _rng: RNG) {
  if (!state.chemistry) initChemistry(state);
  for (const fid of Object.keys(FRANCHISES)) {
    recomputeLeaders(state, fid);
 ...
 */
export function chemistryMultiplier(state: GameState, fid: string): number {
...
export function chemistrySeasonReset(state: GameState) {
...
export function leaderLabel(state: TeamChemistry | undefined): string {
  if (!state) return '—';
  const v = state.value;
