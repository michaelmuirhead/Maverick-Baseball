...
import type { GameState, IntlProspect, IntlSigningState, Position, Player, Nationality } from './types';
import { RNG } from './rng';
...
const INTL_POSITIONS: Position[] = ['C', 'SS', '2B', '3B', 'CF', 'LF', 'RF', '1B', 'SP', 'SP', 'RP'];

function makeProspect(rng: RNG, _yearOpen: number): IntlProspect {
  const nat: Nationality = rng.weightedPick([
  ...
  let mean: number;
  let bonus: number;
  if (tierRoll < 0.02) { mean = 50; bonus = rng.int(4_500_000, 7_500_000); }      // elite
  else if (tierRoll < 0.10) { mean = 42; bonus = rng.int(2_200_000, 4_500_000); } // top
  else if (tierRoll < 0.35) { mean = 35; bonus = rng.int(600_000, 2_200_000); }   // solid
  else if (tierRoll < 0.75) { mean = 28; bonus = rng.int(100_000, 700_000); }     // speculative
  else { mean = 22; bonus = rng.int(10_000, 120_000); }                           // dart-throw
  ...
  if (isPitcher) {
    ...
    ratings.overall = Math.round(
      ratings.contact * 0.24 + ratings.power * 0.22 + ratings.speed * 0.15 +
...
export function startIntlSigningWindow(state: GameState, rng: RNG) {
  ...
  const poolsSpent: Record<string, number> = {};
  for (const fid of Object.keys(FRANCHISES)) {
    ...
    let pool = 5_500_000;
    if (market.tier === 'mega') pool = 4_500_000;
    else if (market.tier === 'large') pool = 5_200_000;
    else if (market.tier === 'mid') pool = 6_000_000;
    else pool = 6_800_000;
...
function materializeProspect(state: GameState, prospect: IntlProspect, fid: string, bonus: number) {
  const player: Player = {
    id: prospect.id,
  ...
  if (!state.prospects.includes(player.id)) state.prospects.push(player.id);
...
export function userSignIntlProspect(
  state: GameState,
  prospectId: string,
  bonus: number,
): { ok: boolean; reason?: string } {
  const intl = state.intlSignings;
  if (!intl || !intl.open) return { ok: false, reason: 'Window closed' };
  const prospect = intl.prospects.find((p) => p.id === prospectId);
  if (!prospect || prospect.signedBy) return { ok: false, reason: 'Unavailable' };
  const fid = state.userFranchiseId;
  const remaining = (intl.pools[fid] || 0) - (intl.poolsSpent[fid] || 0);
  if (bonus > remaining) return { ok: false, reason: `Only $${Math.round(remaining / 1000)}K remaining in pool` };
  if (bonus < prospect.bonusAsk * 0.75) return { ok: false, reason: `${prospect.firstName} ${prospect.lastName} is asking at least $${Math.round(prospect.bonusAsk * 0.75 / 1000)}K` };
...
export function simAIIntlSigningBurst(state: GameState, rng: RNG) {
  const intl = state.intlSignings;
  if (!intl || !intl.open) return;
  ...
  for (let i = 0; i < signingsToday; i++) {
    ...
    for (const fid of teams) {
      const remaining = (intl.pools[fid] || 0) - (intl.poolsSpent[fid] || 0);
      if (remaining < prospect.bonusAsk) continue;
      const phil = effectivePhilosophy(state, fid);
      let weight = 1;
      if (phil === 'rebuilding' || phil === 'retooling') weight = 1.5;
      else if (phil === 'win_now' || phil === 'contending') weight = 0.7;
...
export function closeIntlSigningWindow(state: GameState) {
  if (!state.intlSignings) return;
