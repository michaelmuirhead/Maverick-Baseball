...
export function ensureFatigue(p: Player) {
  if (!p.fatigue) p.fatigue = { lastAppearance: -999, consecutiveDays: 0, pitchCount: 0 };
}

export function recordAppearance(p: Player, day: number, pitchCount = 0) {
  ensureFatigue(p);
...
export function effectiveOvrWithFatigue(p: Player, currentDay: number): number {
  ensureFatigue(p);
  const f = p.fatigue!;
  const daysSince = currentDay - f.lastAppearance;
  let penalty = 0;
  if (daysSince === 1) {
    if (f.consecutiveDays >= 2) penalty = 8;
    else penalty = 3;
...
export function decayFatigue(state: GameState) {
  for (const pid of Object.keys(state.players)) {
    const p = state.players[pid];
    if (!p.fatigue) continue;
    const daysSince = state.day - p.fatigue.lastAppearance;
    if (daysSince >= 3) p.fatigue.consecutiveDays = 0;
  }
}
