...
export const POSITIONS_HITTER: Position[] = ['C', '1B', '2B', '3B', 'SS', 'LF', 'CF', 'RF', 'DH'];

export const MLB_STRUCTURE: { pos: Position; count: number }[] = [
...
export function genPlayer(rng: RNG, franchiseId: string | null, pos: Position): Player {
  ...
  if (pos === 'IF') position = rng.pick(['2B', '3B', 'SS'] as Position[]);
  else if (pos === 'OF') position = rng.pick(['LF', 'CF', 'RF'] as Position[]);
...
export function genRoster(rng: RNG, franchiseId: string, opts: { isExpansion?: boolean } = {}): Player[] {
  const players: Player[] = [];
  for (const slot of MLB_STRUCTURE) {
    for (let i = 0; i < slot.count; i++) players.push(genPlayer(rng, franchiseId, slot.pos));
  }
  for (let i = 0; i < 2; i++) players.push(genPlayer(rng, franchiseId, rng.pick(POSITIONS_HITTER)));
  ...
  if (opts.isExpansion) {
    for (const p of players) {
      if (p.ratings.overall > 62) {
        ...
        if (p.ratings.contact) p.ratings.contact = Math.max(35, p.ratings.contact - excess);
        if (p.ratings.power) p.ratings.power = Math.max(35, p.ratings.power - excess);
        if (p.ratings.fastball) p.ratings.fastball = Math.max(35, p.ratings.fastball - excess);
        if (p.ratings.breaking) p.ratings.breaking = Math.max(35, p.ratings.breaking - excess);
      }
    }
  }
  return players;
