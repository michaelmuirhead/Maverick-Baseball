...
import type { GameState, OwnerProfile, OwnerStyle } from './types';
...
export function generateOwner(rng: RNG, season: number): OwnerProfile {
  const r = rng.next();
  const style: OwnerStyle = r < 0.30 ? 'cheap' : r < 0.75 ? 'balanced' : 'spendthrift';
...
export function initOwnerProfiles(state: GameState, rng: RNG) {
...
export function maybeRollOwnerChanges(state: GameState, rng: RNG) {
  if (!state.ownerProfiles) return;
  for (const fid of Object.keys(FRANCHISES)) {
    if (fid === state.userFranchiseId) continue;       // user is the owner
    const owner = state.ownerProfiles[fid];
    if (!owner) continue;
    const yearsHeld = state.season - owner.acquiredSeason;
    if (yearsHeld < 4) continue;
    if (rng.chance(0.03)) {
...
export function ownerBidMultiplier(state: GameState, fid: string): number {
  const o = state.ownerProfiles?.[fid];
  if (!o) return 1.0;
  if (o.style === 'spendthrift') return 1.18;
  if (o.style === 'cheap') return 0.82;
  return 1.0;
}
