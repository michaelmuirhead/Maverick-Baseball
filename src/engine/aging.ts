...
function ageMultiplier(age: number, isPitcher: boolean): number {
  if (isPitcher) {
    if (age <= 24) return 1.005;     // gentle rise for young arms
    if (age <= 30) return 1.0;        // peak window
    if (age <= 33) return 0.985;      // -1.5%/yr
    if (age <= 35) return 0.97;       // -3.0%/yr
    if (age <= 37) return 0.95;
    return 0.92;                       // 38+ steep decline
  }
  // Hitters
  if (age <= 25) return 1.005;
  if (age <= 29) return 1.0;
  if (age <= 32) return 0.99;
  if (age <= 34) return 0.975;
  if (age <= 36) return 0.955;
  return 0.92;
}
 ...
 */
export function ageAllPlayers(state: GameState, rng: RNG): { aged: number; retired: number; bloomed: number } {
  let aged = 0, retired = 0, bloomed = 0;

  for (const pid of Object.keys(state.players)) {
    const p = state.players[pid];
    if (p.retired) continue;
    if (p.prospect) continue;       // prospects in minors don't age the same way
    ...
    if (p.isPitcher) {
      // Velo declines first
      if (p.ratings.velo) p.ratings.velo = Math.max(70, Math.round(p.ratings.velo * (mult > 1 ? 1.001 : (mult + 0.005))));
      if (p.ratings.fastball) p.ratings.fastball = Math.round(Math.max(20, p.ratings.fastball * mult));
      if (p.ratings.breaking) p.ratings.breaking = Math.round(Math.max(20, p.ratings.breaking * Math.max(mult, 0.95)));
      if (p.ratings.changeup) p.ratings.changeup = Math.round(Math.max(20, p.ratings.changeup * Math.max(mult, 0.96)));
      // Command holds longer
      if (p.ratings.command) p.ratings.command = Math.round(Math.max(20, p.ratings.command * (mult > 1 ? mult : Math.max(mult + 0.01, 0.96))));
      if (p.ratings.movement) p.ratings.movement = Math.round(Math.max(20, p.ratings.movement * mult));
      if (p.ratings.stamina) p.ratings.stamina = Math.round(Math.max(15, p.ratings.stamina * mult));
    } else {
      // Speed declines fastest
      if (p.ratings.speed) p.ratings.speed = Math.round(Math.max(20, p.ratings.speed * Math.min(mult, 0.99)));
      // Power second
      if (p.ratings.power) p.ratings.power = Math.round(Math.max(20, p.ratings.power * Math.max(mult, 0.93)));
      // Contact slowest
      if (p.ratings.contact) p.ratings.contact = Math.round(Math.max(20, p.ratings.contact * Math.max(mult, 0.95)));
      if (p.ratings.eye) p.ratings.eye = Math.round(Math.max(20, p.ratings.eye * (mult > 1 ? mult : Math.max(mult + 0.015, 0.96))));
      if (p.ratings.fielding) p.ratings.fielding = Math.round(Math.max(20, p.ratings.fielding * Math.max(mult, 0.94)));
      if (p.ratings.arm) p.ratings.arm = Math.round(Math.max(20, p.ratings.arm * Math.max(mult, 0.95)));
    }
    ...
    if (Math.abs(p.ratings.overall - wasOVR) > 0) aged++;
  }

  return { aged, retired, bloomed };
}

function retirePlayer(state: GameState, p: Player) {
  p.retired = true;
