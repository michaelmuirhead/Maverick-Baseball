...
const TARGET_PER_LEVEL = { aaa: 28, aa: 28, a: 30 };
export function emptyMinors() {
    return { aaa: [], aa: [], a: [] };
}
export function initMinorRosters(state) {
    state.minorRosters = state.minorRosters || {};
    for (const fid of Object.keys(FRANCHISES)) {
        if (!state.minorRosters[fid])
            state.minorRosters[fid] = emptyMinors();
    }
...
export function levelForProspect(p) {
    if (p.age >= 22 && p.ratings.overall >= 50)
        return 'aaa';
    if (p.age >= 21 && p.ratings.overall >= 42)
        return 'aa';
    if (p.age >= 20 && p.potential >= 55)
        return 'aa';
    return 'a';
}
function isInAnyMinor(state, pid) {
    const ms = state.minorRosters || {};
    for (const fid of Object.keys(ms)) {
        const m = ms[fid];
        if (m.aaa.includes(pid) || m.aa.includes(pid) || m.a.includes(pid))
            return true;
    }
    return false;
}
function findMinorPlayer(state, pid) {
    const ms = state.minorRosters || {};
    for (const fid of Object.keys(ms)) {
        const m = ms[fid];
        if (m.aaa.includes(pid))
            return { fid, level: 'aaa' };
        if (m.aa.includes(pid))
            return { fid, level: 'aa' };
        if (m.a.includes(pid))
            return { fid, level: 'a' };
    }
...
export function callUpFromMinors(state, playerId, opts = {}) {
    const found = findMinorPlayer(state, playerId);
    if (!found)
        return { ok: false, reason: 'Player not in minor leagues' };
...
export function sendDownToMinors(state, playerId, targetLevel = 'aaa') {
    const p = state.players[playerId];
    if (!p || !p.franchiseId)
        return { ok: false, reason: 'Player not on roster' };
 ...
 */
export function maintainAIRoster(state, fid, _rng) {
    ...
    for (const pos of missingPos) {
        ...
        if (candidate)
            callUpFromMinors(state, candidate.id, { enforceDepthFloor: true });
    }
    if (spCount < 5) {
        ...
        const sps = minors.aaa
            ...
            .slice(0, need);
        for (const sp of sps)
            callUpFromMinors(state, sp.id, { enforceDepthFloor: true });
    }
    if (rpCount < 5) {
        ...
        const rps = minors.aaa
            ...
            .slice(0, need);
        for (const rp of rps)
            callUpFromMinors(state, rp.id, { enforceDepthFloor: true });
    }
    // Top up to 26
    if (state.rosters[fid].length < 26) {
        ...
        const best = minors.aaa
            ...
            .slice(0, slotsNeeded);
        for (const p of best)
            callUpFromMinors(state, p.id, { enforceDepthFloor: true });
    }
}
/**
 ...
 */
export function promoteMinorLeaguers(state, rng) {
    const ms = state.minorRosters;
    if (!ms)
        return;
    for (const fid of Object.keys(ms)) {
...
export function minorLeagueDevelopment(state, rng) {
    const ms = state.minorRosters;
    if (!ms)
        return;
    for (const fid of Object.keys(ms)) {
...
export function minorRosterCounts(state, fid) {
    const m = state.minorRosters?.[fid];
    if (!m)
        return { aaa: 0, aa: 0, a: 0 };
    return { aaa: m.aaa.length, aa: m.aa.length, a: m.a.length };
}
import { genPlayer } from './players';
const FILLER_POSITIONS = ['C', '1B', '2B', '3B', 'SS', 'LF', 'CF', 'RF', 'SP', 'SP', 'RP', 'RP'];
/**
 ...
 */
export function seedMinorLeagueFillers(state, rng) {
    for (const fid of Object.keys(state.minorRosters || {})) {
        ...
        for (const lvl of ['aaa', 'aa', 'a']) {
            ...
            while (minors[lvl].length < t.count) {
                ...
                if (!state.prospects.includes(p.id))
