...
export function generateSchedule(rng: RNG): { schedule: Game[]; byDay: Record<number, number[]> } {
  ...
  // Build pairwise series counts. Each "series" is 3 games at one home park.
  type Series = { home: string; away: string; games: number };
  ...
  // a fixed pattern.
  const isOffDay = (d: number) => d % 14 === 0;     // ~13 off-days per season

  const schedule: Game[] = [];
  let gid = 0;

  function tryPlaceSeries(series: Series): boolean {
