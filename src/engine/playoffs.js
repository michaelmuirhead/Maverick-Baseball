...
export function seedBracket(state) {
    const leagueSeeds = { AL: [], NL: [] };
    for (const lg of ['AL', 'NL']) {
        const byDiv = { East: [], Central: [], West: [] };
        for (const fid of Object.keys(FRANCHISES)) {
            if (FRANCHISES[fid].lg === lg)
                byDiv[FRANCHISES[fid].div].push(fid);
        }
        const divWinners = [];
...
export function simSeriesGame(state, series, rng) {
    if (series.status !== 'active' || !series.higherSeed || !series.lowerSeed)
        return series;
    const gameNum = series.games.length + 1;
    let homeFid;
    if (series.atHigher === 'all')
        homeFid = series.higherSeed.id;
    else if (series.atHigher === '2-2-1')
        homeFid = (gameNum <= 2 || gameNum === 5) ? series.higherSeed.id : series.lowerSeed.id;
    else if (series.atHigher === '2-3-2')
        homeFid = (gameNum <= 2 || gameNum >= 6) ? series.higherSeed.id : series.lowerSeed.id;
    else
        homeFid = series.higherSeed.id;
    ...
    if (winnerId === series.higherSeed.id)
        series.higherWins++;
    else
        series.lowerWins++;
    series.games.push({
...
export function advancePostseasonDay(state, rng) {
    const bracket = state.bracket;
    if (!bracket || bracket.champion)
        return state;
    const s = { ...state, bracket: { ...bracket, series: { ...bracket.series } } };
    ...
    for (const id of Object.keys(sBracket.series)) {
        ...
        if (series.status !== 'complete' || series._promoted)
            continue;
        if (series.round === 'wild_card') {
            ...
            if (champStaff && state.coaches) {
                for (const role of ['manager', 'pitching_coach', 'hitting_coach']) {
                    ...
                    if (cid && state.coaches[cid])
                        state.coaches[cid].championships += 1;
                }
            }
        }
        ...
        const roundLabel = series.round === 'wild_card' ? 'Wild Card Series' :
    ...
    if (sBracket.champion)
        sBracket.currentRound = 'complete';
    else if (allLcsDone)
        sBracket.currentRound = 'world_series';
    else if (allDsDone)
        sBracket.currentRound = 'lcs';
    else if (allWcDone)
        sBracket.currentRound = 'division';
    return s;
}
