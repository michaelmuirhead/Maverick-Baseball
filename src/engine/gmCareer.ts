 ...
 */
export function generateGMJobOffers(state: GameState, rng: RNG) {
  ...
  const offers: NonNullable<GameState['gmJobOffers']> = [];
...
export function acceptGMOffer(state: GameState, franchiseId: string): { ok: boolean; reason?: string } {
  if (!state.fired) return { ok: false, reason: 'You are still employed' };
