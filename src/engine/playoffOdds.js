...
export function pythagoreanWins(rf, ra, gamesPlayed, totalGames = 162) {
    if (rf === 0 && ra === 0)
        return Math.round(totalGames * 0.5);
    const ratio = Math.pow(rf, 1.83) / (Math.pow(rf, 1.83) + Math.pow(ra, 1.83));
    return Math.round(ratio * totalGames);
}
export function projectFinalRecord(s, played, total = 162) {
    if (played === 0)
        return { w: 0, l: 0 };
 ...
 */
export function magicNumber(state, fid, totalGames = 162) {
    const team = state.standings[fid];
    if (!team)
        return null;
    const f = FRANCHISES[fid];
    const divRivals = Object.keys(FRANCHISES).filter((id) => id !== fid && FRANCHISES[id].lg === f.lg && FRANCHISES[id].div === f.div);
 ...
 */
export function playoffOdds(state, fid, totalGames = 162) {
    const team = state.standings[fid];
    if (!team)
        return 0;
