...
export function waivePlayer(state: GameState, playerId: string): { ok: boolean; deadMoney: number; reason?: string } {
  const p = state.players[playerId];
  if (!p) return { ok: false, deadMoney: 0, reason: 'Player not found' };
  if (!p.franchiseId) return { ok: false, deadMoney: 0, reason: 'Player not on a roster' };
  const fid = p.franchiseId;
  const f = FRANCHISES[fid];
  if (!f) return { ok: false, deadMoney: 0, reason: 'Franchise not found' };
  ...
  if (!state.freeAgents.includes(playerId)) state.freeAgents.push(playerId);
