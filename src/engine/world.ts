...
import {
  chooseAutoTeamSlot, generateExpansionTeam, registerExpansionFranchise,
} from './expansion';
...
export function initWorld(
  ...
  let autoTeamFid: string | null = null;
  ...
  const gmPhilosophies: GameState['gmPhilosophies'] = {};
  ...
  if (expansionConfig) {
    for (const fid of [userFid, autoTeamFid!] as string[]) {
...
function newSeasonReset(state: GameState, rng: RNG) {
  for (const fid of Object.keys(state.standings)) {
    state.standings[fid] = { wins: 0, losses: 0, rf: 0, ra: 0, streak: 0, l10: [] } as Standing;
  }
  const { schedule, byDay } = generateSchedule(rng);
...
export function advanceDay(state: GameState): GameState {
  ...
  if (newState.phase === 'regular_season') {
    ...
    if (daysToDeadline >= 0) {
      let shouldSim = false;
      if (daysToDeadline <= 3) shouldSim = rng.chance(0.9);
      else if (daysToDeadline <= 10) shouldSim = rng.chance(0.5);
      else if (daysToDeadline <= 30) shouldSim = rng.chance(0.18);
      else shouldSim = rng.chance(0.05);
      if (shouldSim) newState = simAITradeActivity(newState, rng);
    }
  }
...
export function advanceToMilestone(state: GameState, milestone: string): GameState {
  let s = state;
  const start = s.day;
  const startSeason = s.season;
  for (let i = 0; i < 365; i++) {
    s = advanceDay(s);
    if (milestone === 'next_day' && i >= 0) return s;
    if (milestone === 'next_week' && ((s.season === startSeason && s.day - start >= 7) || s.season > startSeason)) return s;
    if (milestone === 'opening_day' && s.day === 1 && s.phase === 'regular_season') return s;
