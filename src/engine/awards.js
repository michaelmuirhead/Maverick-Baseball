...
export function awardSeason(state) {
    ...
    for (const lg of ['AL', 'NL']) {
        ...
        const FIELDING_POS = ['C', '1B', '2B', '3B', 'SS', 'LF', 'CF', 'RF'];
...
function pushAward(p, season, type, league) {
    p.awards = p.awards || [];
...
export function hofScore(p) {
    ...
    for (const a of p.awards) {
        if (a.type === 'mvp')
            score += 100;
        else if (a.type === 'cy_young')
            score += 90;
        else if (a.type === 'rookie')
            score += 25;
        else if (a.type === 'gold_glove')
            score += 12;
    }
    if (p.careerHigh)
        score += Math.max(0, p.careerHigh - 60) * 5;
    return score;
}
/** Run HoF voting at season rollover. Inducts retired players past cooldown. */
export function processHallOfFame(state) {
    const inductionThreshold = 250; // tuned for ~5% induction rate
    for (const pid of Object.keys(state.players)) {
        const p = state.players[pid];
        if (!p.retired || p.hallOfFame)
            continue;
        if (!p.retiredOn)
            continue;
        if (state.season - p.retiredOn < HOF_RETIREMENT_COOLDOWN)
            continue;
...
export function awardFreeAgentMultiplier(p) {
    if (!p.awards)
        return 1.0;
    let mult = 1.0;
    for (const a of p.awards) {
