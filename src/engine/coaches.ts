...
import type { Coach, CoachRole, GameState, FranchiseCoaches, Player, DevelopmentRecord } from './types';
import { pickName, NAT_WEIGHTS } from './names';
import { FRANCHISES } from './franchises';

let COACH_ID_COUNTER = 0;
function newCoachId(): string { return `c_${(++COACH_ID_COUNTER).toString(36)}`; }
...
export function generateCoach(rng: RNG, role: CoachRole): Coach {
  ...
  const baseSalary = ({ elite: 4_500_000, star: 2_400_000, solid: 1_200_000, fringe: 700_000 } as const)[tier as 'elite' | 'star' | 'solid' | 'fringe'];

  const style = role === 'manager'
    ? rng.pick(['small_ball', 'analytical', 'vet_friendly', 'balanced'] as const)
    : undefined;
...
export function signCoach(
  ...
  if (!state.staffByFid) state.staffByFid = {};
...
export function fireCoach(state: GameState, coachId: string): { ok: boolean; buyout: number; reason?: string } {
  const coach = state.coaches?.[coachId];
  if (!coach || !coach.franchiseId) return { ok: false, buyout: 0, reason: 'No coach found' };
  ...
  if (!state.coachPool.includes(coachId)) state.coachPool.push(coachId);

  return { ok: true, buyout };
}

/** Initialize coaches at world init: every team gets a fresh staff. */
export function initCoaches(state: GameState, rng: RNG) {
  resetCoachIdCounter();
  state.coaches = {};
  state.staffByFid = {};
  state.coachPool = [];
  ...
  for (const fid of Object.keys(FRANCHISES)) {
    const staff: FranchiseCoaches = { manager: null, pitching_coach: null, hitting_coach: null };
    for (const role of ['manager', 'pitching_coach', 'hitting_coach'] as CoachRole[]) {
  ...
  for (let i = 0; i < 20; i++) {
    const role = rng.pick(['manager', 'pitching_coach', 'hitting_coach'] as CoachRole[]);
    const coach = generateCoach(rng, role);
    state.coaches[coach.id] = coach;
...
export function rolloverCoaches(state: GameState, rng: RNG) {
  if (!state.coaches || !state.staffByFid) return;
  ...
  for (const id of Object.keys(state.coaches)) {
    ...
    if (c.contract && c.franchiseId) {
      ...
      if (c.contract.years <= 0) {
        ...
        c.contract = null;
        state.coachPool = state.coachPool || [];
        if (!state.coachPool.includes(id)) state.coachPool.push(id);
      }
    }
  }
 ...
 */
export function aiFireStaff(state: GameState, rng: RNG) {
  for (const fid of Object.keys(FRANCHISES)) {
    if (fid === state.userFranchiseId) continue;
    const lastSeasonRecord = state.finances[fid]?.history?.slice(-1)[0]?.record;
    if (!lastSeasonRecord) continue;
    ...
    if (staff.manager && rng.chance(mgrFireChance)) fireCoach(state, staff.manager);
    if (staff.pitching_coach && rng.chance(cdFireChance)) fireCoach(state, staff.pitching_coach);
    if (staff.hitting_coach && rng.chance(cdFireChance)) fireCoach(state, staff.hitting_coach);
  }
}

/** Score a coach for an AI hiring decision (higher = better fit). */
function aiCoachScore(coach: Coach, role: CoachRole): number {
  if (coach.role !== role) return -1;
  return coach.ratings.overall + coach.experience * 0.5 - Math.max(0, coach.age - 60);
}

/** AI teams fill empty staff slots from the pool. */
export function aiHireStaff(state: GameState, _rng: RNG) {
  for (const fid of Object.keys(FRANCHISES)) {
    // If user has delegated staff hiring, AI fills their slots too.
    if (fid === state.userFranchiseId && !state.delegateStaffHiring) continue;
    const staff = state.staffByFid?.[fid];
    if (!staff) continue;
    for (const role of ['manager', 'pitching_coach', 'hitting_coach'] as CoachRole[]) {
      if (staff[role]) continue;        // slot filled
...
export function managerBonus(state: GameState, fid: string): number {
  const mgrId = state.staffByFid?.[fid]?.manager;
  if (!mgrId) return -1.5;       // no manager = real penalty
...
export function chemistryBonus(state: GameState, fid: string): number {
  const mgrId = state.staffByFid?.[fid]?.manager;
  if (!mgrId) return 0;
  const mgr = state.coaches?.[mgrId];
  if (!mgr) return 0;
...
export function pitchingCoachInjuryMult(state: GameState, fid: string): number {
...
export function developmentBonus(state: GameState, fid: string, isPitcher: boolean): number {
  const role: CoachRole = isPitcher ? 'pitching_coach' : 'hitting_coach';
 ...
 */
export function projectedBump(state: GameState, p: Player): number {
  if (!p.franchiseId) return 0;
...
export function coachDevelopmentLog(state: GameState, coachId: string): DevelopmentRecord[] {
...
export function franchiseDevelopmentLog(state: GameState, fid: string): DevelopmentRecord[] {
...
export function developableYoungPlayers(state: GameState, fid: string): Player[] {
 ...
 */
export function topDevelopingCoaches(state: GameState, limit = 10) {
  const totals: Record<string, { coachId: string; bumps: number; totalPotentialGain: number; totalOverallGain: number }> = {};
...
// ---------------------------------------------------------------------------
// Tactical impact — the manager's tactical rating, surfaced as concrete
// per-position-group contributions used by sim.ts and the Staff page.
// ---------------------------------------------------------------------------

export interface TacticalContribution {
  /** Manager's raw tactical rating, or 50 if no manager. */
  tactical: number;
  /** Normalized to roughly [-1, +1]: -1 = poor, 0 = average, +1 = elite. */
  tacticalNorm: number;
  /** Bench leverage delta in OVR points (extra hitter rating extracted from bench). */
  benchLeverage: number;
  /** Bullpen leverage delta in OVR points (closer/setup weighted vs back-end). */
  bullpenLeverage: number;
  /** Intangibles bonus (small flat add to teamStrength). */
  intangibles: number;
}
...
function styleMultipliers(style?: import('./types').ManagerStyle) {
  switch (style) {
...
export function computeTacticalContribution(state: GameState, fid: string): TacticalContribution {
  const mgrId = state.staffByFid?.[fid]?.manager;
 ...
 */
export function managerIntangibleBonus(state: GameState, fid: string): number {
  const mgrId = state.staffByFid?.[fid]?.manager;
  const mgr = mgrId ? state.coaches?.[mgrId] : null;
  if (!mgr) return -0.6;
