...
export function teamStrength(state, fid) {
...
export function simGame(rng, state, game) {
    ...
    if (actualHome !== desiredHome) {
        if (desiredHome && homeScore <= awayScore)
            homeScore = awayScore + rng.int(1, 3);
        else if (!desiredHome && homeScore >= awayScore)
            awayScore = homeScore + rng.int(1, 3);
    }
    if (homeScore === awayScore) {
        if (rng.next() < homeWinProb)
            homeScore++;
        else
            awayScore++;
    }
...
export function applyResult(state, game, rng) {
    if (rng)
        recordGameStats(state, game, rng);
    const r = game.result;
