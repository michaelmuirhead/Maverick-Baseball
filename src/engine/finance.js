...
export function teamPayroll(state, fid) {
    return (state.rosters[fid] || []).reduce((s, pid) => s + (state.players[pid]?.contract?.salary || 0), 0);
}
export function computeLuxuryTax(annualPayroll) {
    if (annualPayroll <= CBT_TIER1)
        return 0;
    let tax = 0;
...
export function cbtStatus(payroll) {
    const tax = computeLuxuryTax(payroll);
    let nextTier = null;
    if (payroll < CBT_TIER1)
        nextTier = { name: 'Tier 1 (20%)', threshold: CBT_TIER1, distance: CBT_TIER1 - payroll };
    else if (payroll < CBT_TIER2)
        nextTier = { name: 'Tier 2 (32%)', threshold: CBT_TIER2, distance: CBT_TIER2 - payroll };
    else if (payroll < CBT_TIER3)
        nextTier = { name: 'Tier 3 (62.5%)', threshold: CBT_TIER3, distance: CBT_TIER3 - payroll };
...
export function genFinance(rng, franchise) {
    const market = MARKETS[franchise.market];
    const tvBase = { mega: 120_000_000, large: 70_000_000, mid: 42_000_000, small: 28_000_000 }[market.tier];
    const tv = Math.round(tvBase * rng.float(0.8, 1.25));
    const nameRights = rng.chance(0.7)
        ? Math.round({ mega: 20, large: 9, mid: 5, small: 3 }[market.tier] * 1_000_000 * rng.float(0.8, 1.4))
        : 0;
    const sponsors = Math.round((3_000_000 + market.corp / 80 * 8_000_000) * 4 * rng.float(0.8, 1.2));
    const tierMult = { mega: 1.4, large: 1.15, mid: 1.0, small: 0.85 }[market.tier];
    ...
    const valMult = { mega: 9.5, large: 7.0, mid: 5.0, small: 3.8 }[market.tier];
...
export function gameDayRev(rng, franchise, finance, wins, losses, day, rivalry) {
    const market = MARKETS[franchise.market];
...
export function applyAccruals(state) {
    for (const fid of Object.keys(state.finances)) {
        ...
        if (fin.debt > 0)
            fin.ledger.debtService += Math.round(fin.debt * 0.055 / 365);
        if (state.day >= 1 && state.day <= 183) {
            const annualPay = state.rosters[fid].reduce((s, pid) => s + (state.players[pid]?.contract?.salary || 0), 0);
            fin.ledger.payroll += Math.round(annualPay / 183);
        }
        fin.ledger.staff += Math.round(55_000_000 / 365);
    }
}
export function finalizeSeason(state, rng) {
    awardSeason(state);
    ...
    for (const fid of fids) {
        ...
        l.cbtTax = cbtTax;
        const revenue = l.ticketRev + l.premiumRev + l.concRev + l.parkRev + l.merchRev +
            l.localTV + l.natTV + l.sponsorRev + l.namingRev + (l.postseasonRev || 0);
        const expenses = l.payroll + l.staff + l.stadium + l.minors + l.scouting + l.analytics +
    ...
    for (const r of provisional) {
        ...
        if (fin.tvYearsLeft <= 0) {
            ...
            const base = { mega: 135_000_000, large: 80_000_000, mid: 48_000_000, small: 32_000_000 }[market.tier];
        ...
        const valMult = { mega: 9.5, large: 7.0, mid: 5.0, small: 3.8 }[MARKETS[FRANCHISES[r.fid].market].tier];
        fin.franchiseValue = Math.round(r.revenue * valMult + FRANCHISES[r.fid].champ * 30_000_000 - fin.debt);
    }
}
