...
import type { GameState, Player, HitterStats, PitcherStats, SeasonStats } from './types';
...
function lineupRoleAB(p: Player, pos: string): { games: number; abPerGame: number } {
  // Catchers play less (defensive load); regulars 130-155 G; bench 50-100 G.
  if (pos === 'C') return { games: 130, abPerGame: 3.6 };
...
export function generateHitterStats(p: Player, rng: RNG, teamRecord: { w: number; l: number }): HitterStats {
...
export function generatePitcherStats(p: Player, rng: RNG, teamRecord: { w: number; l: number }): PitcherStats {
  const ovr = p.ratings.overall;
  ...
  if (isSP) {
    ...
    if (isCloser) SV = Math.round(clamp(35 - (5 - era) * -3 + rng.normal(0, 4), 5, 50));
    else if (ovr >= 60) SV = Math.round(rng.int(0, 5));
  }
 ...
 */
export function generateSeasonStats(state: GameState, rng: RNG) {
...
export function careerHitterTotals(p: Player): HitterStats | null {
...
export function careerPitcherTotals(p: Player): PitcherStats | null {
