...
import type { GameState, HitterStats, PitcherStats } from './types';
...
const DEFAULT_LEAGUE: LeagueAverages = {
  OPS: 0.730, ERA: 4.20, AVG: 0.252, OBP: 0.320, SLG: 0.410,
};

export function computeLeagueAverages(state: GameState): LeagueAverages {
  ...
  if (totalAB === 0) return DEFAULT_LEAGUE;
...
export function woba(h: HitterStats): number {
...
export function babip(h: HitterStats): number {
...
/** ISO: pure power. */
export function iso(h: HitterStats): number {
  return Math.round((h.SLG - h.AVG) * 1000) / 1000;
}

/** OPS+: 100 = league avg. (player_OPS / lg_OPS) * 100, with park-neutral assumption. */
export function opsPlus(h: HitterStats, lg: LeagueAverages): number {
  if (h.OPS === 0 || lg.OPS === 0) return 0;
...
export function eraPlus(p: PitcherStats, lg: LeagueAverages): number {
  if (p.ERA === 0 || p.IP === 0) return 0;
  const v = 100 * (lg.ERA / p.ERA);
  return Math.round(v);
}

/** FIP: defense-independent ERA. (13*HR + 3*BB - 2*K) / IP + constant */
export function fip(p: PitcherStats, lg: LeagueAverages): number {
  if (p.IP === 0) return 0;
...
export function hitterWAR(h: HitterStats, lg: LeagueAverages): number {
  if (h.AB < 50) return 0;
...
export function pitcherWAR(p: PitcherStats, lg: LeagueAverages): number {
  if (p.IP < 20) return 0;
...
export function attachAdvanced(season: number, lg: LeagueAverages, hStats?: HitterStats, pStats?: PitcherStats) {
...
export function recomputeAllAdvanced(state: GameState) {
