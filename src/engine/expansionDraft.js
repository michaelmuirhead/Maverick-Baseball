 ...
 */
export function runExpansionDraft(state, expansionFids, rng) {
    if (expansionFids.length === 0)
        return;
    // Collect unprotected players from each existing (non-expansion) team
    const unprotected = [];
    for (const fid of Object.keys(FRANCHISES)) {
        if (FRANCHISES[fid].expansion)
            continue; // skip expansion teams themselves
        ...
        for (const p of roster) {
            if (!protectedIds.has(p.id))
                unprotected.push({ fromFid: fid, player: p });
        }
    }
    ...
    const picksByExpansion = {};
    for (const fid of expansionFids)
        picksByExpansion[fid] = [];
    // Snake order: Team A, Team B, Team B, Team A, A, B, B, A, ...
    const totalPicks = expansionFids.length * PICKS_PER_EXPANSION_TEAM;
    let availIdx = 0;
    const teamsTakenFrom = {}; // limit 1 player per existing team per expansion
    for (let pick = 0; pick < totalPicks; pick++) {
        ...
        if (chosen <= availIdx)
            availIdx = Math.max(0, availIdx - 1);
    }
    ...
    for (const expFid of expansionFids) {
        ...
        for (const p of picks) {
            if (!state.rosters[expFid].includes(p.id))
                state.rosters[expFid].push(p.id);
        }
        // Dropped players become free agents (cut)
        for (const p of dropped) {
            p.franchiseId = null;
            state.freeAgents = state.freeAgents || [];
            if (!state.freeAgents.includes(p.id))
